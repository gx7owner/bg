...
async def golem(update: Update, context: CallbackContext):
    ...
    if len(args) != 3:
        await context.bot.send_message(chat_id=chat_id, text="*⚠️ Tere ko simple command bhi nahi aati? Chal, sikh le: /coins <add|rem> <user_id> <coins>*", parse_mode='Markdown')
        return
